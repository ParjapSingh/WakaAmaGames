...
class WakaAmaApp(tk.Tk):
    ...
    # export to csv file code
    
    def export_to_csv(self):
        filename = f"WakaAmaResults{self.selected_year}.csv"

        # Make sure we sort correctly by points (int) in descending order
        sorted_results = sorted(self.points.items(), key=lambda item: int(item[1]), reverse=True)
